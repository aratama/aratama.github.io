...
function init(config) {
    ...
    if (recordAsOGG) {
        vorbisEncoder = new VorbisEncoder({
            vorbisLibPath: config.vorbisLibPath
        })
...
var MP3Encoder = function(config) {
    ...
    function init(config) {
        var channels = config.cannels || 1;
        var sampleRate = config.sampleRate || 48000;
        var bitRate = config.bitRate || 128;
        console.log(JSON.stringify({
            channels: channels,
            sampleRate: sampleRate,
            bitRate: bitRate
        }));
        mp3encoder = new lib.Mp3Encoder(channels, sampleRate, bitRate); //mono 44.1khz encode to 128kbps
    }
